function [resultVC,resultVE]=inter_route_1_insert(VC,VE,vehicle,demands,comparts,classes,a,b,L,s,dist,centernum,alpha,gamma,belta)
resultVC=VC;
resultVE=VE;
NV=length(VE);
...
parfor i=1:n
    ...
    if len2==0
        ...
        [num,inpos]=meshgrid(1:len1,1:len2+1);
        comb=[num(:) inpos(:)];
%         index= comb(:,1)~=comb(:,2);
%         comb=comb(index,:);
        nn=size(comb,1);
        for j=1:nn
            tempR1=route1;
            tempR1(comb(j,1))=[];
            tempR2=[route2(1:comb(j,2)-1) route1(comb(j,1)) route2(comb(j,2):end)];
            cost1=costM(tempR1,cap,duration,demands,comparts,classes,a,b,L,s,dist,center1(i),centernum,alpha,gamma,belta);
            cost2=costM(tempR2,cap,duration,demands,comparts,classes,a,b,L,s,dist,center2(i),centernum,alpha,gamma,belta);
            tempCost=cost1+cost2;
            if cost(i)<constCost-tempCost
