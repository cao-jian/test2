function [resultVC,resultVE]=inter_route_swap(VC,VE,vehicle,demands,comparts,classes,a,b,L,s,dist,centernum,alpha,gamma,belta)
resultVC=VC;
resultVE=VE;
NV=length(VE);
...
parfor i=1:n
    ...
    nn=size(comb,1);
    for j=1:nn
        temp=route1(comb(j,1));
        tempR1=route1;
        tempR1(comb(j,1))=route2(comb(j,2));
        tempR2=route2;
        tempR2(comb(j,2))=temp;
